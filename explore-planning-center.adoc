...
您可以选择迁移计划并进行配置，以便将虚拟机迁移到您选择的 Amazon AWS 环境。
...
** link:deploy-gcnv-datastores.html["为 Google Cloud VMware Engine 套餐部署 Google Cloud NetApp Volumes 数据存储区"]
...
=== 创建迁移计划的 PDF 或 CSV 报告

生成一份包含迁移计划详细信息的 PDF 或 CSV 报告。
...
. 选择*迁移计划*。
. 选择要创建 PDF 或 CSV 报告的迁移计划的“操作”菜单 (...)，然后选择“下载计划 (PDF)”或“下载 VM 报告 (CSV)”。
. 输入报告名称，然后选择“保存”。
